#reference: ('https://www.yoheim.net/blog.php?q=20160201')

import sys, sqlite3
from collections import namedtuple
from pprint import pprint

conn = sqlite3.connect('/Users/kitagawashinya/Desktop/wnjpn.db')
Word = namedtuple('Word', 'wordid lang lemma pron pos')


def getWords(lemma):
    cur = conn.execute('select * from word where lemma=?',
                       (lemma,))
...
def getSenses(word):
    cur = conn.execute('select * from sense where wordid=?',
                       (word.wordid,))
...
def getSynset(synset):
    cur = conn.execute('select * from synset where synset=?',
                       (synset,))
